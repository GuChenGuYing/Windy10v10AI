...
function modifier_item_dragon_lance_plus:GetModifierBonusStats_AttackRange()
	if self:GetAbility() then
		return self:GetAbility():GetSpecialValueFor("bonus_attack_range")
	end 
end 

function modifier_item_dragon_lance_plus:GetModifierBonus_AttackSPEED()
	if self:GetAbility() and self:GetAbility():GetSecondaryCharges() == 1 then
		local current_int = self:GetParent():GetGetAgility()
...
function modifier_item_dragon_lance_plus_active:GetModifierSpellAmplify_Percentage()
