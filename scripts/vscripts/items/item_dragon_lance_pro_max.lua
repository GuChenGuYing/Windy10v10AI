...
function modifier_item_dragon_lance_pro_max:GetModifierBonusStats_AttackRange()
	if self:GetAbility() then
		return self:GetAbility():GetSpecialValueFor("bonus_attack_range")
	end 
end 

function modifier_item_dragon_lance_pro_max:GetModifierBonus_AttackSPEED()
	if self:GetAbility() and self:GetAbility():GetSecondaryCharges() == 1 then
		local current_int = self:GetParent():GetAgility()
		local applied_amp = current_int * self:GetAbility():GetSpecialValueFor("active_boost_int")
		return applied_amp
	end 
end 
...
function modifier_item_dragon_lance_pro_max_active:GetModifierSpellAmplify_Percentage()
